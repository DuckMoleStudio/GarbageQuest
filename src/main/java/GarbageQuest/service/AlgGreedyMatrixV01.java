...
public class AlgGreedyMatrixV01 {
    public static Result Calculate(List<WayPoint> wayPoints, List<MatrixLine> matrix, int avgSpeed, int carCapacity){

        Result result = new Result();
        result.setMethodUsed("Simple Greedy Algorithm, matrix distance calculation, V.01");
        ...
        //List<MatrixLine> matrix = Matrix.FillSimple(wayPoints); // initialize "distances" matrix
        ...
        while (!wayPoints.isEmpty())
        {
            ...
            while (!subWP.isEmpty())
            {
                ...
                MatrixElement me = Matrix.Nearest(curWP, matrix, subWP);
                        ...
                        curDistance/avgSpeed)),tryWP.getTimeClose()).getSeconds()>0)
                {
                    ...
                    if(curWait <= 0)
                    {
                        ...
                        if(++curLoad == itinerary.getCar().getCapacity())
                        {
                            ...
                            curDistance = Matrix.DistanceBetween(curWP,dump,matrix);
