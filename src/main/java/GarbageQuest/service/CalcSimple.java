...
public class CalcSimple {
    public static Result Calculate(List<WayPoint> wayPoints){
        final int AVG_SPEED = 10; // 10 m/s is about 30 km/h

        Result result = new Result();
        result.setMethodUsed("Simple Greedy Algorithm");
        ...
        while (!wayPoints.isEmpty())
        {
                ...
                itinerary.setCar(new Car("Kamaz Othodov # "+(++itCount), WayPointType.Garbage_Site, 5));
                ...
                while (!subWP.isEmpty())
                {
                    ...
                    if(Duration.between(curTime.plus(Duration.ofSeconds((long)
                            curDistance/AVG_SPEED)),tryWP.getTimeClose()).getSeconds()>0)
                    {
                        curWait = Duration.between
                                (curTime.plus(Duration.ofSeconds((long)curDistance/AVG_SPEED)),tryWP.getTimeOpen()).getSeconds();
                        ...
                        if(curWait <= 0)
                        {
                            ...
                            curTime = curTime.plus(Duration.ofSeconds((long)curDistance/AVG_SPEED)); // enroute time
